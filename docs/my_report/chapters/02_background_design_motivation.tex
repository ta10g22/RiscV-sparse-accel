...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
\caption{Representative density ranges for four workload classes. Recommender and GNN matrices (social/citation graphs) exhibit extreme sparsity ($<0.1\%$), while pruned CNNs retain 10--20\% density. By operating only on non-zero elements, SpMM avoids 90\% to $>99.99\%$ of the arithmetic required by dense multiplication.}
